...
class MasterProblem:
    ...
    def _build_model(self):
        ...
        self.model.setParam('DualReductions', 0) # Infeasibleの診断用
    ...
    def solve(self) -> Tuple[float, Dict, Dict]:
        """
        マスター問題を解く
        Infeasibleの場合はfeasRelaxを使って緩和問題を解く
        """
        self.model.update()
        self.model.optimize()
        ...
        if self.model.Status == GRB.INFEASIBLE:
            logger.warning("  マスター問題が実行不可能。feasRelaxを実行して解を探索します...")
            
            # feasRelaxS: 制約違反を最小化するような解を見つける
            # 第1引数: 0=目的関数変更なし, 1=緩和解へ
            # 第2引数: 緩和の可否(False=変数の上下限は守る)
            # 第3引数: 制約違反のペナルティ重み(Trueで全制約)
            # 第4引数: 変数上限違反のペナルティ
            self.model.feasRelaxS(1, False, True, False)
            self.model.optimize()
        
        # それでも解けない場合の最終手段（安全装置）
        if self.model.Status not in [GRB.OPTIMAL, GRB.SUBOPTIMAL]:
            logger.error(f"  マスター問題解決失敗。Status: {self.model.Status}")
