...
class CentralizedOptimizer:
    ...
    def setup_model(self):
        ...
        # ========== 決定変数 ==========
        # アグリゲータレベル
        self.P_ag_buy = self.model.addVars(range(1, self.T+1), lb=0, name="P_ag_buy") # アグリゲータの系統からの買電電力[kW]
        self.P_ag_sell = self.model.addVars(range(1, self.T+1), lb=0, name="P_ag_sell") # アグリゲータの系統への売電電力[kW]

        # 需要家レベル
        self.P_buy = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, ub=self.BUY_MAX, name="P_buy") # 一軒の需要家の買電電力量[kW*30min]
        self.P_sell = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, ub=self.SELL_MAX, name="P_sell") # 一軒の需要家の売電電力量[kW*30min]
        self.delta_buy = self.model.addVars(range(1, self.T+1), range(self.H), vtype=GRB.BINARY, name="delta_buy") # P_buyのフラグ
        self.delta_sell = self.model.addVars(range(1, self.T+1), range(self.H), vtype=GRB.BINARY, name="delta_sell") # P_sellのフラグ
        ...
        # 電圧（配電網側）
        self.V = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="Voltage") # 電圧[V]
        self.V_dd = self.model.addVars(range(1, self.T+1), range(self.H), lb=-GRB.INFINITY, name="V_dd") # 電圧変化[V]
        self.V_UN = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="V_UN") # 電圧上限セーフ値[V]
        self.V_LN = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="V_LN") # 電圧下限セーフ値[V]
        self.V_ULV = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="V_ULV") # 電圧上限違反量[V]
        self.V_LLV = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="V_LLV") # 電圧下限違反量[V]
        ...
        self.S_DN = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="S_DN") # 送電線容量[VA]
        self.S_UN = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="S_UN") # 送電線容量セーフ値[VA]
        self.S_ULV = self.model.addVars(range(1, self.T+1), range(self.H), lb=0, name="S_ULV") # 送電線容量上限違反量[VA]
    ...
    def _add_power_balance_constraints(self):
        ...
        for t in range(1, self.T+1):
            ...
            for h in range(self.H):
                ...
                # 潮流計算用
                self.model.addConstr(
                    self.P_H[t, h] == self.P_sell[t, h] - self.P_buy[t, h],
                    name=f"P_H_{t}_{h}"
                )
                self.model.addConstr(
    ...
    def _add_voltage_constraints(self):
        ...
        for t in range(1, self.T+1):
            for h in range(self.H):
                ...
                # 電圧制約（ペナルティ変数付き）
                self.model.addConstr(
                    self.V_UL - self.V[t, h] == self.V_UN[t, h] - self.V_ULV[t, h], # 電圧上限違反
                    name=f"voltage_upper_{t}_{h}"
                )
                self.model.addConstr(
                    self.V[t, h] - self.V_LL == self.V_LN[t, h] - self.V_LLV[t, h], # 電圧下限違反
    ...
    def _set_objective(self):
        ...
        # ペナルティ項（電圧・送電線容量の違反量に対するペナルティ）
        penalty = gp.quicksum(
            self.V_UN[t, h] + self.V_ULV[t, h] + self.S_ULV[t, h]
            for t in range(1, self.T+1) for h in range(self.H)
        )

        # 目的関数設定
        self.model.setObjective(
            electricity_cost + ddr_total + penalty,
            GRB.MINIMIZE
        )
    ...
    def plot_results(self, results: Dict):
        ...
        for h in range(self.H):
            ...
            ax.set_title(f'Household {h}: Voltage')
            ax.legend(loc='upper right')
        ...
        logger.info(f"分析グラフを {output_dir} に保存しました。")
# ==================== メイン実行 ====================
def main():
    """メイン実行関数"""
    try:
