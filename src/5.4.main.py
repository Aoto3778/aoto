...
class MasterProblem:
        ...
        for f in range(F):
            for t in range(1, T+1):
                self.model.addConstr(
                    self.P_f[f, t] <= self.params.BUY_MAX,
                    name=f'feeder_limit_{f}_{t}'
                )
        
        # ★ 初期カット: eta >= 下限値（非有界を防ぐ）
        self.model.addConstr(
            self.eta >= -500.0,  # 合理的な下限
            name='eta_initial_bound'
        )
        
        self.model.update()
        
        logger.info("主問題を構築しました")
    ...
    def add_optimality_cut(self, cut_constant: float, cut_gradient: np.ndarray, P_f_current: np.ndarray):
        ...
        η >= Z*_sub + Σ μ*_t · (P^f,t - P^f,t_fixed)
        
        Args:
            cut_constant: Z*_sub（部分問題の最適目的関数値）
            cut_gradient: μ*_t（電力配分制約の双対変数）
            P_f_current: P^f,t_fixed（現在の点）
        """
        F = self.params.F
        T = self.params.T
        ...
        for f in range(F):
            for t in range(1, T+1):
                gradient_val = cut_gradient[f, t-1]
                P_f_diff = self.P_f[f, t] - P_f_current[f, t-1]
                cut_expr += gradient_val * P_f_diff
        
        # ★ デバッグ情報
        logger.debug(f"  【最適性カット】")
        logger.debug(f"    定数項 Z*_sub: {cut_constant:.2f}")
        logger.debug(f"    勾配 μ の範囲: [{cut_gradient.min():.4f}, {cut_gradient.max():.4f}]")
        logger.debug(f"    P_f_current の範囲: [{P_f_current.min():.4f}, {P_f_current.max():.4f}]")
        
        # ★ 異常値チェック
        if np.max(np.abs(cut_gradient)) > 1e6:
            logger.warning(f"    警告: 勾配が異常に大きい（最大={np.max(np.abs(cut_gradient)):.2e}）")
        
        if abs(cut_constant) > 1e6:
            logger.warning(f"    警告: 定数項が異常に大きい（{cut_constant:.2e}）")
        ...
        self.optimality_cuts.append({
            ...
            'gradient': cut_gradient.copy(),
            'point': P_f_current.copy()
        })
        
        self.model.update()
        
        logger.info(f"  最適性カット追加（計{len(self.optimality_cuts)}個）")
    
    def add_feasibility_cut(self, cut_gradient: np.ndarray, P_f_current: np.ndarray, violation: float):
        """実行可能性カット追加（式5-4'に基づく）
        
        Σ π*_t · (P^f,t - P^f,t_fixed) ≤ -w*
        
        Args:
            cut_gradient: π*_t（実行可能性SPの電力配分制約の双対変数）
            P_f_current: P^f,t_fixed（現在の点）
            violation: w*（スラック変数の和）
        """
        F = self.params.F
        T = self.params.T
        
        # カットの式を構築
        cut_expr = -violation  # 右辺を左辺に移項
        ...
        logger.debug(f"  【実行可能性カット】")
        logger.debug(f"    違反量 w*: {violation:.4f}")
        logger.debug(f"    勾配 π の範囲: [{cut_gradient.min():.4f}, {cut_gradient.max():.4f}]")
        ...
        logger.info(f"  実行可能性カット追加（計{len(self.feasibility_cuts)}個）")
    ...
    def solve(self) -> Tuple[np.ndarray, float]:
        ...
        if self.model.status == gp.GRB.OPTIMAL:
            ...
            logger.info(f"  主問題の解:")
            logger.info(f"    eta = {eta_value:.2f}")
            logger.info(f"    P_f 平均 = {P_f_solution.mean():.4f} kW")
            logger.info(f"    P_f 最大 = {P_f_solution.max():.4f} kW")
            
            return P_f_solution, eta_value
        
        elif self.model.status == gp.GRB.UNBOUNDED:
            logger.error("主問題が非有界です")
            
            # 非有界診断
            self.model.setParam('InfUnbdInfo', 1)
            self.model.optimize()
            
            logger.info("  非有界方向の変数:")
            
            # eta をチェック
            try:
                if abs(self.eta.UnbdRay) > 1e-6:
                    logger.info(f"    eta: UnbdRay = {self.eta.UnbdRay:.4f}")
                    logger.error("    → eta が非有界です。最適性カットが不足しています！")
            except:
                pass
            ...
            for f in range(self.params.F):
                for t in range(1, self.params.T+1):
                    try:
                        ray = self.P_f[f, t].UnbdRay
                        if abs(ray) > 1e-6:
                            logger.info(f"    P_f[{f},{t}]: UnbdRay = {ray:.4f}")
                    except:
                        pass
            
            self.model.write("master_unbounded.lp")
            raise RuntimeError("主問題が非有界です")
        
        elif self.model.status == gp.GRB.INFEASIBLE:
            logger.error("主問題が実行不可能です")
            
            # IIS計算
            try:
                self.model.computeIIS()
                self.model.write("master_infeasible.ilp")
                ...
                self.model.write("master_infeasible.lp")
            
            raise RuntimeError("主問題が実行不可能です")
        
        else:
            logger.error(f"主問題が予期しないステータス: {self.model.status}")
            self.model.write("master_error.lp")
            raise RuntimeError(f"主問題のステータス異常: {self.model.status}")
...
class Subproblem:
    ...
    def build(self, P_f_fixed: np.ndarray):
        ...
        V_UN = model.addVars(range(1, T+1), range(H), name='V_UN', lb=0.0, ub=gp.GRB.INFINITY)
        V_LN = model.addVars(range(1, T+1), range(H), name='V_UN', lb=0.0, ub=gp.GRB.INFINITY)
        V_ULV = model.addVars(range(1, T+1), range(H), name='V_ULV', lb=0.0, ub=gp.GRB.INFINITY)
        V_LLV = model.addVars(range(1, T+1), range(H), name='V_LLV', lb=0.0, ub=gp.GRB.INFINITY)
        ...
        for t in range(1, T+1):
            for k in range(H):
                c1 = model.addConstr(
                    V_d[t,k] - self.params.V_LL == V_LN[t,k],
                    name=f'V_lower_{t}_{k}'
                )
                c2 = model.addConstr(
                    self.params.V_UL - V_d[t,k] == V_UN[t, k],
...
class FeasibilitySubproblem:
    ...
    def build(self, P_f_fixed: np.ndarray):
        ...
        V_ULV = model.addVars(range(1, T+1), range(H), name='V_ULV', lb=0.0, ub=gp.GRB.INFINITY)
        V_LLV = model.addVars(range(1, T+1), range(H), name='V_LLV', lb=0.0, ub=gp.GRB.INFINITY)
        S_over = model.addVars(range(1, T+1), range(H), name='S_over', lb=0.0, ub=gp.GRB.INFINITY)
        ...
        # スラック変数の合計を最小化
        obj = gp.quicksum(
            V_ULV[t,k] + V_LLV[t,k] + S_over[t,k]
            for t in range(1, T+1)
            for k in range(H)
        )
        model.setObjective(obj, gp.GRB.MINIMIZE)
        ...
        for t in range(1, T+1):
            for k in range(H):
                c1 = model.addConstr(
                    V_d[t,k] - self.params.V_LL == V_LN[t,k] - V_LLV[t, k],
                    name=f'V_lower_{t}_{k}'
                )
                c2 = model.addConstr(
                    self.params.V_UL - V_d[t,k] == V_UN[t, k] - V_ULV[t,k],
                    name=f'V_upper_{t}_{k}'
                )
                voltage_lower.append(c1)
                voltage_upper.append(c2)
...
class BendersSolver:
    ...
    def solve(self) -> Dict:
        ...
        while self.iteration < self.params.MAX_ITERATIONS:
            ...
            # ========== Step 2: MPの解法と更新 ==========
            logger.info("\n主問題(MP)を解いています...")
            P_f_new, eta_value = self.master.solve()
            ...
            for f in range(self.params.F):
                for t in range(self.params.DOWN_DR_START_HOUR, self.params.DOWN_DR_END_HOUR + 1):
                    if 1 <= t <= self.params.T:
                        P_f_dr_sum += P_f_new[f, t-1]
            
            # 下界 LBD = 主問題の目的関数値 = (Σ P_f + eta)
            self.LBD = P_f_dr_sum + eta_value
            
            logger.info(f"\n【下界の計算】")
            logger.info(f"  P_f（下げDR期間） = {P_f_dr_sum:.2f}")
            logger.info(f"  eta（電力コスト代理変数） = {eta_value:.2f}")
            logger.info(f"  LBD = P_f + eta = {self.LBD:.2f}")
            logger.info(f"  P_PG の平均 = {P_f_new.mean():.4f} kW")
            logger.info(f"  P_PG の最大 = {P_f_new.max():.4f} kW")
            
            # ========== 収束判定 ==========
            logger.info(f"\n{'='*60}")
            logger.info(f"【収束状況（反復 {self.iteration}）】")
            logger.info(f"{'='*60}")
            logger.info(f"  上界 UBD = {self.UBD:.4f}")
            logger.info(f"  下界 LBD = {self.LBD:.4f}")
            
            if self.UBD != float('inf') and self.LBD != float('-inf'):
                gap = abs(self.UBD - self.LBD)
                relative_gap = gap / abs(self.UBD) if abs(self.UBD) > 1e-10 else float('inf')
                
                logger.info(f"  絶対ギャップ = {gap:.4f}")
                logger.info(f"  相対ギャップ = {relative_gap*100:.4f}%")
                logger.info(f"  収束閾値 = {self.params.EPSILON:.4f}")
                
                # 収束判定
                if gap < self.params.EPSILON:
                    logger.info(f"\n{'='*60}")
                    logger.info(f"★ 収束しました！（反復 {self.iteration} 回）")
                    logger.info(f"★ 最適目的関数値: {self.UBD:.4f}")
                    logger.info(f"{'='*60}")
                    self.optimal_objective = self.UBD
                    break
                elif relative_gap < self.params.EPSILON:
                    logger.info(f"\n{'='*60}")
                    logger.info(f"★ 収束しました（相対ギャップ）！（反復 {self.iteration} 回）")
                    logger.info(f"★ 最適目的関数値: {self.UBD:.4f}")
                    logger.info(f"{'='*60}")
                    self.optimal_objective = self.UBD
                    break
            else:
                logger.info(f"  ギャップ計算不可（UBDまたはLBDが無限大）")
            ...
            self.convergence_history.append({
                ...
                'UBD': self.UBD if self.UBD != float('inf') else None,
                'LBD': self.LBD if self.LBD != float('-inf') else None,
                'gap': gap if self.UBD != float('inf') and self.LBD != float('-inf') else None
            })
        ...
        if self.iteration >= self.params.MAX_ITERATIONS:
            logger.warning(f"\n最大反復回数（{self.params.MAX_ITERATIONS}）に達しました")
            if self.UBD != float('inf'):
                logger.info(f"最良の上界: {self.UBD:.4f}")
                self.optimal_objective = self.UBD
    ...
    def plot_convergence(self):
        ...
        plt.tight_layout()
        plt.savefig('../data/output/optimization/benders_convergence.png', dpi=150)
        plt.show()
