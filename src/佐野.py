...
cph = 16.2 # HP給湯機加熱能力[MJ/h]
cw = 0.0042 # 水の比熱[MJ/m3/℃]
ce = 3.6 # 変換係数[MJ/kWh]
xhp = 0.013 # HP給湯機補機消費電力[kWh]
vlt = 370 # 貯水槽の容量[L]
r1 = 0.05
for day in range(0,365):
    ...
    cost = {}
    for t in range(1,T+1):
        cost[t] = HEMS_IND.addVar(vtype=vt, name=vn + '({:})'.format(t), lb=LB, ub=UB)
    ...
    for t in range(0,T+1):
        for i in range(1,DN+1):
            H_prod[i,t] = HEMS_IND.addVar(vtype=vt, name=vn + '({:},{:})'.format(i, t), lb=LB, ub=UB)
    ...
    for t in range(1,T+1):
        for i in range(1,DN+1):
            H_ini[i,t] = HEMS_IND.addVar(vtype=vt, name=vn + '({:},{:})'.format(i, t), lb=LB, ub=UB)
    ...
    for t in range(0,T+1):
        for i in range(1,DN+1):
            H_tank[i,t] = HEMS_IND.addVar(vtype=vt, name=vn + '({:},{:})'.format(i, t), lb=LB, ub=UB)
    ...
    for t in range(0,T+1):
        for i in range(1,DN+1):
            Delta_prod[i,t] = HEMS_IND.addVar(vtype=vt,name=vn+'({:},{:})'.format(i,t))
    ...
    for t in range(0,T+1):
        for i in range(1,DN+1):
            Delta_S[i,t] = HEMS_IND.addVar(vtype=vt,name=vn+'({:},{:})'.format(i,t))
    ...
    for t in range(0,T+1):
        for i in range(1,DN+1):
            Delta_F[i,t] = HEMS_IND.addVar(vtype=vt,name=vn+'({:},{:})'.format(i,t))
    ...
    for t in range(1, T+1):
        for i in range(1, DN+1):  # 1から8の範囲
            for j in range(1, DN+1):
                P_fp[i,j,t] = HEMS_IND.addVar(vtype=vt, name=vn + '({:},{:},{:})'.format(i, j, t), lb=LB, ub=UB)   
                #iからjに供給
...
# 最終目的関数: コスト + 電圧違反量の重み付け和
    ...
    if day >= 1:
        ...
        for i in range(1,DN+1):
            ...
            (H_tank[i,0] == float(previous_result.iat[int(T/2-1),20]))
            (Delta_prod[i,0] == previous_result.iat[int(T/2-1),15])
            (Delta_S[i,0] == previous_result.iat[int(T/2-1),16])
            (Delta_F[i,0] == previous_result.iat[int(T/2-1),17])
    ...
    for t in range(1, T+1):     
        for i in range(1, DN+1):
            HEMS_IND.addConstr(P_out[i,t]== P_sg[i, t] + gp.quicksum(P_fp[i,j,t] for j in range(1, DN+1)))
            HEMS_IND.addConstr(P_in[i,t]== P_pg[i, t] + gp.quicksum(P_fp[j,i,t] for j in range(1, DN+1)))    
            HEMS_IND.addConstr(P_fp[i,i,t]==0)
    ...
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(P_DCh[i,t] <= N_B_PCS * Delta_dch[i,t])
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(P_Ch[i,t] <= N_B_PCS * Delta_ch[i,t])
    for t in range(1,T+1):
        for i in range(1,DN+1):
    ...
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(N_B*0.2 <= E_B[i,t])
            HEMS_IND.addConstr(E_B[i,t] <= N_B)
    ...
    # 蓄電池蓄電量始端・終端制約##
    for i in range(1,DN+1):
         HEMS_IND.addConstr(E_B[i,0] == 0.5*N_B)
         HEMS_IND.addConstr(E_B[i,T] == 0.5*N_B)

    # HP給湯機に関する制約条件
    # !HP給湯機の消費電力[kWh]と生成する熱量[MJ]に関する等式制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(ce*cop[t]*(P_HP[i,t] - xhp*Delta_F[i,t]) == H_prod[i,t] + H_ini[i,t])

    #! 生成する熱量に関する上下限制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(r1*cph*Delta_prod[i,t] <= H_prod[i,t]) # 定格出力で3分以上運転する
        
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_prod[i,t] <= cph * Delta_prod[i,t])
    #! 起動時の運転継続制約（運転を開始した場合は最低でも 1 時間は運転を継続する制約）
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_prod[i,t] + H_prod[i,t-1] >= cph * Delta_prod[i,t])
        
    #! 運転開始時のエネルギーロス
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_ini[i,t] == 0.01*cph*Delta_S[i,t])
            
    #! 貯湯槽内の蓄熱量更新に関する制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_tank[i,t] - H_tank[i,t-1] == H_prod[i,t] - dmh[t])
            
    #! 運転状態の更新制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(Delta_prod[i,t] - Delta_prod[i,t-1] == Delta_S[i,t] - Delta_F[i,t])
            
    #! 起動と停止の同時禁止制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(Delta_S[i,t] + Delta_F[i,t] <= 1)

    #! 運転開始後定格運転を行う制約
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_prod[i,t] >= cph*(Delta_S[i,t] - Delta_prod[i,t]))
        
    #! 貯湯槽の容量制約（沸き上げ温度65度）
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(H_tank[i,t] >= 0.2*(cw*70*vlt))
            HEMS_IND.addConstr(H_tank[i,t] <= (cw*70*vlt))
        
    # !HP給湯機の運転に関する制約条件
    # 運転中    
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(Delta_S[i,t] <= Delta_prod[i,t])
    #!
    for t in range(1,T+1):
        for i in range(1,DN+1):
            HEMS_IND.addConstr(Delta_F[i,t] + Delta_prod[i,t]<= 1)


    # 前回の最適化を初期値にする制約条件##
    for i in range(1,DN+1):
         HEMS_IND.addConstr(H_tank[i,0] == H_tank[i,T])

    #時間当たりの電力コスト
    for t in range(1,T+1):
        HEMS_IND.addConstr(cost[t] == buy_priENERGY_CONVERSION_FACTOR[t]*P_PG[t]-sell_priENERGY_CONVERSION_FACTOR*P_SG[t])
...
# 電力価格（双対変数）
    ...
    for i in range(1,line+1):
        ...
        if i<=6:
            ...
            eneflow['cost_{:}'.format(i)] = [cost[t].X for t in range(1,48+1)]
            ...
            eneflow['Delta_r_{:}'.format(i)] = [Delta_prod[i,t].X for t in range(1,48+1)]
            eneflow['Delta_st_{:}'.format(i)] = [Delta_S[i,t].X for t in range(1,48+1)]
            eneflow['Delta_sp_{:}'.format(i)] = [Delta_F[i,t].X for t in range(1,48+1)]
            eneflow['H_HS_{:}'.format(i)] = [H_prod[i,t].X for t in range(1,48+1)]
            eneflow['H_EL_{:}'.format(i)] = [H_ini[i,t].X for t in range(1,48+1)]
            eneflow['H_SC_{:}'.format(i)] = [H_tank[i,t].X for t in range(1,48+1)]
            ...
            eneflow['P_vv8_1_{:}'.format(i)] = [P_fp[1,i,t].X for t in range(1,48+1)]
            eneflow['P_vv8_2_{:}'.format(i)] = [P_fp[2,i,t].X for t in range(1,48+1)] 
            eneflow['P_vv8_3_{:}'.format(i)] = [P_fp[3,i,t].X for t in range(1,48+1)] 
            eneflow['P_vv8_4_{:}'.format(i)] = [P_fp[4,i,t].X for t in range(1,48+1)] 
            eneflow['P_vv8_5_{:}'.format(i)] = [P_fp[5,i,t].X for t in range(1,48+1)] 
            eneflow['P_vv8_6_{:}'.format(i)] = [P_fp[1,i,t].X for t in range(1,48+1)] 
            eneflow['P_vv8_total_{:}'.format(i)] = [sum(P_fp[j, i, t].X for j in range(1, 6+1)) for t in range(1, 48+1)]
