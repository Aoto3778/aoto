...
for t in range(1, T+1):
    for k in range(H):
        # 上限・下限違反制約
        AG_PM.addConstr(V_UL - V_d[t,k] == V_UN[t,k])
        AG_PM.addConstr(V_d[t,k] - V_LL == V_LN[t,k])
...
for t in range(1, T+1):
    for k in range(H):
        ...
        # 送電線の定格電力の二次錐制約
        # SOC: ||[ap, aq]||_2 <= S_LINE  <=>  ap^2 + aq^2 <= S_LINE^2
        AG_PM.addQConstr(ap[t, k]*ap[t, k] + aq[t, k]*aq[t, k] <= S_LINE * S_LINE)
...
for i in range(H):
    ...
    # CSVファイルとして保存（出力先は共通の output_dir を利用）
